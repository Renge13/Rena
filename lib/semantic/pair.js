...
export function buildPairSemantic(chartA, chartB, { engineVersion = ENGINE_VERSION } = {}) {
  ...
  // ── P0. THERE IS NO p0_names FACT, AND THAT IS A DECISION. ──
  // The opening obligation - BOTH archetype names, in the first block - is real
  // and is enforced by `both_named` against `core.a` and `core.b`. It does not
  // need a FACT, and it turned out it must not have one:
  //
  //  1. NO CELL WAS RULED FOR IT. `docs/content/compat-glossary-rulings.md` has
  //     24 cells and none of them opens a reading. Both names come from
  //     `arketipe`, which is already ruled.
  //  2. A FACT WITH NO CELL HARD-FAILS STAGE 6. With `entry: null` the fact has
  //     `label: null`, which the glossary reserves for CONDITIONS, and
  //     `blockFor` then wrote "Kamu adalah Matahari (The Sun)." into the floor -
  //     a name-with-bracket on a condition, which `fact.condition_named` HARD
  //     rejects. Measured, not predicted: it was the one hard finding standing
  //     between the applied rulings and a servable floor.
  //  3. THE CLAUSE COULD ONLY NAME ONE PERSON ANYWAY. `RENDER_COPY.floorIdentity`
  //     is "Kamu adalah" + "dengan unsur" - a single-subject sentence. Naming
  //     both would need a connective for the second person, which is new
  //     user-facing Indonesian and therefore Reyner's alone (rule 20). Inventing
  //     one to make a test pass is the exact thing this repo forbids.
  //
  // **SO THE FLOOR DOES NOT NAME EITHER PERSON, AND THAT IS AN OPEN GAP** - not
  // a defect in this file and not something to fix by writing a sentence. It is
  // recorded in the PR and in NEXT.md as owed: one ruled opening cell would close
  // it. The RENDERED path is unaffected; the compat prompt requires both names in
  // the first block and `both_named` enforces it.
